...
@click.option("-lmin", "--lmin", help="ignore sequences w/length < lmin", required= False, type = int, default = 26000)
...
@click.option("-c", "--country", help="Country", required= False, default ='')
...
def main(lmin, lmax, xambig, gisaid_sequences, gisiad_metadata, sample_lineage, country, region, fasta_output, metadata_output1, metadata_output2):
    ...
    if (country != ''):
        df_subset = df.loc[((df['Pango lineage'] == sample_lineage) & df['Location'].str.contains('North America') & df['Location'].str.contains(country)), :]
    else:
        df_subset = df.loc[((df['Pango lineage'] == sample_lineage) & df['Location'].str.contains('North America')), :]
