...
option_list <- list(make_option(c("-n", "--newick_file"), type="character", help="Phylogentic Tree in Newick Formart", metavar="newick_file"),
                    make_option(c("-l", "--lineage_report"), type="character", help="lineage report generated by pangolin", metavar="lineage_report"),
                    make_option(c("-a", "--alleles_info"), type="character", help="Alleles Inforamtion", metavar="alleles_info"))
...
plot_tree_with_snps <- function(tree, alleles, lineage)
{
  ...
  cols <- c("blue", "red", "green", "purple", "grey")
...
if(!interactive()) {
    ...
    tree_path <- opt$newick_file
    tree <- read.tree(tree_path)
    
    alleles_path <- opt$alleles_info
    alleles <- read.table(alleles_path, header=T)

    lineage_fn <- opt$lineage_report
    lineage <- read.csv(
        file=lineage_fn,
        header=TRUE,
        as.is=TRUE,
